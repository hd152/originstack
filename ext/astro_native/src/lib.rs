...
fn mean(v: &[f32]) -> f32 {
    ...
    // .algebraic_add() (stable 1.98+) lets LLVM reassociate/auto-vectorize
    // this reduction -- plain `Iterator::sum()` is specified to fold
    // strictly left-to-right (required for reproducibility), which blocks
    // SIMD on a float accumulator. Reordering does change last-bit rounding
    // vs the old strict-sum path, same as numpy's own pairwise-summation
    // internals already do vs a naive sum -- existing native-vs-numpy parity
    // tests compare with a tolerance, not bit-exact, for this reason.
    let s: f64 = v.iter().fold(0.0f64, |acc, &x| acc.algebraic_add(x as f64));
...
fn std_pop(v: &[f32]) -> f32 {
    ...
    let m = mean(v) as f64;
    let var: f64 = v.iter().fold(0.0f64, |acc, &x| {
        let d = x as f64 - m;
        acc.algebraic_add(d.algebraic_mul(d))
    }) / n as f64;
    var.sqrt() as f32
}
...
fn std_sample(v: &[f32]) -> f32 {
    ...
    let m = mean(v) as f64;
    let var = v.iter().fold(0.0f64, |acc, &x| {
        let d = x as f64 - m;
        acc.algebraic_add(d.algebraic_mul(d))
    }) / (n as f64 - 1.0);
    var.sqrt() as f32
...
fn std_pop_f64(v: &[f64]) -> f64 {
    ...
    let m: f64 = v.iter().fold(0.0f64, |acc, &x| acc.algebraic_add(x)) / n as f64;
    let var: f64 = v.iter().fold(0.0f64, |acc, &x| {
        let d = x - m;
        acc.algebraic_add(d.algebraic_mul(d))
    }) / n as f64;
...
/// length-P dot product using `.algebraic_add()`/`.algebraic_mul()` so LLVM
/// can auto-vectorize the reduction (same technique as this file's `mean`/
/// `std_pop` -- see their comments for why `Iterator::sum()` alone can't).
/// Lower triangle mirrored from the upper (symmetric by construction).
///
        ...
        let results: Vec<((usize, usize), f64)> = pairs
            ...
            .map(|&(i, j)| {
                ...
                let dot = row_i.iter().zip(row_j.iter()).fold(0.0f64, |acc, (&a, &b)| {
                    acc.algebraic_add(a.algebraic_mul(b))
                });
                ((i, j), dot)
            })
            .collect();
...
/// length-N-term linear combination of `data`'s N rows, accumulated with
/// `.algebraic_add()`/`.algebraic_mul()` for the same auto-vectorization
/// reason as `gram_matrix_wide`.
#[pyfunction]
        ...
        out.par_chunks_mut(p).enumerate().for_each(|(k, out_row)| {
            ...
            for i in 0..n {
                ...
                for (o, &d) in out_row.iter_mut().zip(data_row.iter()) {
                    *o = o.algebraic_add(w.algebraic_mul(d));
                }
            }
        });
    });
