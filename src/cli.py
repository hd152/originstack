...
def build_parser() -> argparse.ArgumentParser:
    ...
    g_post.add_argument('--bg-method', choices=['mesh', 'dbe', 'wavelet', 'physical'],
                   ...
                   help='Background extraction method (default: dbe). '
                        ...
                        'physical: EXPERIMENTAL. Fits a first-principles sky model '
                        '(moonlight via Krisciunas-Schaefer, van Rhijn airglow, zodiacal '
                        'light, ground-source skyglow) whose spatial shapes are fixed by '
                        'geometry, leaving one non-negative amplitude per component free, '
                        'so unlike the surface fitters above it has nowhere to put a '
                        'nebula. BUT on a typical ~1 deg deep-sky field the physical '
                        'components are nearly constant across the frame and the model '
                        'cannot fit the real gradient, which at that scale is mostly '
                        'instrumental (vignetting, amp glow); measured on a real Lagoon '
                        'session it made the gradient worse where dbe removed 68%% of it. '
                        'It now measures whether it actually flattened the background and '
                        'falls back to dbe when it did not, so expect it to engage only '
                        'on wide fields. Needs a session info.json with a WCS, GPS and '
                        'timestamp (it runs before --plate-solve).')
