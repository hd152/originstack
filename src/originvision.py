"""originvision integration: in-process defect/quality/category scoring.

originvision is a separately-trained vision classifier. Its ONNX-inference code
is ported into ``src/originvision_infer.py`` (numpy/scipy, no OpenCV) and the
exported model ships inside the package at ``src/data/originvision.onnx`` -- no
external folder, venv or subprocess. ``onnxruntime`` is an optional
dependency; ``--originvision`` self-disables with a warning when it or the model
file is absent.

Advisory/logging only -- results are stored on FrameInfo.metrics['originvision']
for visibility but never set f.accepted or touch f.metrics['score'], and no
frame is auto-dropped. No network -- matches originvision's local-only premise.
"""
...
from src import originvision_infer
...
def run_originvision_infer(image_path: str,
    ...
    """
    return originvision_infer.score_path(image_path, model_path=model_path)
...
def score_lights_with_originvision(lights: List[FrameInfo], args) -> None:
    """Score every accepted light frame with originvision, advisory-only.

    Stores the raw result (or None on failure) at f.metrics['originvision'].
    ...
    Config.ORIGINVISION_OUTLIER_SIGMA below the session mean -- all log-only,
    matching originvision's early/unvalidated integration status.

    Gated on --originvision-score-all, not just --originvision: even in-process,
    ...
    """
    if not (getattr(args, 'originvision', False) and getattr(args, 'originvision_score_all', False)):
        return
    ...
    safe_print(f"\n  originvision: scoring {len(targets)} frame(s) "
               f"({workers} worker(s))...")

    def _score(f: FrameInfo):
        return f, run_originvision_infer(f.path, model_path)
    ...
    if defective:
        safe_print(f"  originvision: {len(defective)} frame(s) flagged is_defective "
                   f"(advisory -- not auto-dropped): "
                   + ", ".join(os.path.basename(p) for p in defective[:5])
                   + (", ..." if len(defective) > 5 else ""))
    if stray:
        safe_print(f"  originvision: {len(stray)} frame(s) flagged stray_light "
                   f"(advisory -- not auto-dropped): "
    ...
    if len(scores) >= 3:
        ...
        if std > 1e-6:
            sigma_thresh = Config.ORIGINVISION_OUTLIER_SIGMA
            below = [paths_scored[i] for i in range(len(scores))
                    if (mean - scores[i]) / std > sigma_thresh]
            if below:
                safe_print(f"  originvision: {len(below)} frame(s) below-session-average "
                           f"quality_score (>{sigma_thresh:.1f}sigma, advisory only): "
                           + ", ".join(os.path.basename(p) for p in below[:5])
                           + (", ..." if len(below) > 5 else ""))


# originvision's category head is a coarse 4-class taxonomy (galaxy, nebula,
...
def map_originvision_category(category: Optional[str]) -> Optional[str]:
    """originvision category -> one of auto_settings.py's target-type anchors,
...
def sample_session_priors(lights: List[FrameInfo], args) -> Optional[dict]:
    """Fast, session-level originvision signal for --auto: a category (fed into
    ...
    scoring every accepted frame (score_lights_with_originvision's job, a
    ...
    Returns None if originvision isn't configured, or every sampled call
    ...
    def _one(i: int):
        return run_originvision_infer(accepted[i].path, model_path)
...
def score_master_with_originvision(master_image_path: str, args,
                               inferred_type: Optional[str] = None) -> None:
    """Score the final stacked master with originvision, advisory-only (log only).
    ...
    ``originvision_infer._load_image_any``, wrong for an already-stacked
    (3, H, W) RGB master.

    Compares originvision's predicted category against the pipeline's own
    ...
    result = run_originvision_infer(master_image_path, model_path)
    if result is None:
        safe_print("  originvision: master scoring failed (see warning above)")
        return
    ...
    safe_print(f"  originvision (master): category={category} "
               f"conf={confidence:.0%}  "
    ...
    if inferred_type and category and inferred_type != 'unknown':
        # originvision's category head is a coarse 4-class taxonomy (galaxy,
        ...
        # even though originvision got it right. Match on shared word tokens
        ...
        if not (_inferred_words & _category_words):
            logger.warning(
                f"originvision: master category '{category}' does not match "
                f"pipeline-inferred target type '{inferred_type}' -- "
                f"possible misidentified/mislabeled session")
            safe_print(f"  originvision WARNING: master category '{category}' "
