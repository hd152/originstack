...
def run_stacking_phase(
    ...
    if getattr(args, 'bm4d_prefilter', False) and drizzle_scale > 1.0:
        safe_print("  WARNING: --bm4d-prefilter is not compatible with "
                   "--drizzle-scale > 1 (needs the batch aligned-stack array the drizzle "
                   "per-frame resample path doesn't build) -- skipped for this run.")
    if use_aligned_memmap:
        ...
        # BM4D cross-frame prefilter: denoises each aligned frame using exact
        # cross-frame correspondence (already established by alignment) --
        # same pipeline slot as --cosmic-ray-rejection/--trail-reject. Runs
        # per-channel: each channel carries independent physical signal
        # pre-combine, unlike bm3d_denoise's post-combine luminance+chroma
        # split (which exists to avoid spatial edge colour-fringing -- not a
        # concern for this temporal-correspondence filter).
        if getattr(args, 'bm4d_prefilter', False):
            _t_bm4d = time.time()
            try:
                from src.bm4d import bm4d_denoise_stack
                for _ch in range(C):
                    mem_aligned[:, :, :, _ch] = bm4d_denoise_stack(
                        np.asarray(mem_aligned[:, :, :, _ch]), verbose=args.verbose)
                mem_aligned.flush()
                safe_print(f"    BM4D prefilter: {n_final} frames x {C} channels "
                          f"({format_time(time.time() - _t_bm4d)})")
            except Exception as _bm4dexc:
                safe_print(f"    WARNING: BM4D prefilter failed: {_bm4dexc}")
