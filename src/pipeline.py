...
def stack_target(frames: List[FrameInfo], output_path: str, args: argparse.Namespace,
    ...
    if resume_phase < 3:
        ...
        try:
            ...
            # --originvision (--originvision-score-all): scoring every accepted frame
            # is a decode + debayer + stretch + resize + forward pass each,
            # which adds up on a large session -- --originvision alone only pays
            # that cost 3x total, via sample_session_priors above.
            if getattr(args, 'originvision', False) and getattr(args, 'originvision_score_all', False):
                from src.originvision import score_lights_with_originvision
                score_lights_with_originvision(lights, args)
    ...
    if getattr(args, 'originvision', False):
        ...
        from src.originvision import score_master_with_originvision
        ...
        score_master_with_originvision(_master_image, args,
