...
| [src/stacking.py](src/stacking.py) | Sigma-clip, percentile, ESD, trimmed mean, Linear Fit Clipping, inverse-variance-weighted, wavelet-subband (`--stack-method wavelet`), drizzle (Lanczos-3 or PSF-matched kernel via `--drizzle-kernel psf`, `iterative_back_projection` IBP super-res refinement via `--super-res-iters`), `run_stacking_phase` |
...
| [src/live_stack.py](src/live_stack.py) | Real-time stacking (`--live`): watches the capture directory and folds each new sub into a running weighted-mean stack, pushing the growing result + running SNR to the web view |
| [src/webview.py](src/webview.py) | `--web-view`: a stdlib-only (`http.server.ThreadingHTTPServer`) local dashboard. `GET /` serves an inline HTML/JS SPA with live phase progress, log stream, per-frame quality ticker, and preview images (zoom/pan/re-stretch/wipe-slider compare) pushed over `GET /events` (Server-Sent Events, polls a version counter) — the SSE snapshot also carries `app_version` (from `read_version()`, `src/utils.py`), which the page reflects in its `<title>` alongside run status. Every `safe_print()`/`print_phase()` call in the codebase tees into it for free (no-op when inactive). `POST /api/start` + `GET /api/schema` (see `src/webview_control.py`) let it also launch a run, not just observe one — CSRF-hardened (strict `application/json` Content-Type so a classic `<form>` POST can never hit it, plus an Origin/Host check) and body-size-capped. `GET /api/health` reports `{native, version}` (whether `astro_native` loaded) — used by `packaging/verify_build.ps1` to confirm a packaged build isn't silently running the numpy fallback. `run_finished()` also fires a best-effort native OS notification (`src/notify.py`) since a stacking run is exactly the kind of thing a user tabs away from |
| [src/webview_control.py](src/webview_control.py) | Desktop-app control layer behind `webview.py`'s `POST /api/start`/`GET /api/schema`: `get_form_schema()` introspects `cli.build_parser()` live (no hand-maintained duplicate of ~120 flags to drift), `build_argv_from_form()` turns a submitted form back into a synthetic argv fed through the real `cli.parse_args()` (so preset/config/`--auto` precedence and `_explicit_cli_dests` come out exactly as from a real command line), and `RunManager`/`get_run_manager()` runs one pipeline job at a time on a background thread. `RunManager.is_running()` backs `desktop_app.py`'s close-confirmation (warns before quitting mid-run) |
| [src/desktop_app.py](src/desktop_app.py) | `python desktop_app.py`: opens `webview.py`'s dashboard in a native `pywebview` window instead of a browser tab. Only extra capability over the plain browser page is native file/folder pickers (`Api.browse_directory`/`browse_output_path`/`browse_path`, exposed as `window.pywebview.api`) — everything else works identically either way. Falls back to opening the dashboard in a browser (and staying alive to serve it) when `pywebview` isn't installed. Every failure path routes through `_fatal()` (log to `%LOCALAPPDATA%\OriginStack\logs\` + a `tkinter` error dialog) — load-bearing for the packaged build (see below), which runs windowed with no console for a bare `print()` to reach. `window.events.closing` is wired to confirm via `RunManager.is_running()` before quitting mid-run |
