...
| [src/webview.py](src/webview.py) | `--web-view`: a stdlib-only (`http.server.ThreadingHTTPServer`) local dashboard. `GET /` serves an inline HTML/JS SPA with live phase progress, log stream, per-frame quality ticker, and preview images (zoom/pan/re-stretch/wipe-slider compare) pushed over `GET /events` (Server-Sent Events, polls a version counter). Every `safe_print()`/`print_phase()` call in the codebase tees into it for free (no-op when inactive). `POST /api/start` + `GET /api/schema` (see `src/webview_control.py`) let it also launch a run, not just observe one — CSRF-hardened (strict `application/json` Content-Type so a classic `<form>` POST can never hit it, plus an Origin/Host check) and body-size-capped |
| [src/webview_control.py](src/webview_control.py) | Desktop-app control layer behind `webview.py`'s `POST /api/start`/`GET /api/schema`: `get_form_schema()` introspects `cli.build_parser()` live (no hand-maintained duplicate of ~120 flags to drift), `build_argv_from_form()` turns a submitted form back into a synthetic argv fed through the real `cli.parse_args()` (so preset/config/`--auto` precedence and `_explicit_cli_dests` come out exactly as from a real command line), and `RunManager`/`get_run_manager()` runs one pipeline job at a time on a background thread |
| [src/desktop_app.py](src/desktop_app.py) | `python desktop_app.py`: opens `webview.py`'s dashboard in a native `pywebview` window instead of a browser tab. Only extra capability over the plain browser page is native file/folder pickers (`Api.browse_directory`/`browse_output_path`/`browse_path`, exposed as `window.pywebview.api`) — everything else works identically either way. Falls back to printing the dashboard URL (and staying alive to serve it) when `pywebview` isn't installed |
