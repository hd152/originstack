...
2. **Phase 2 — Registration** — Coarse-to-fine pyramid seed (half-resolution, integer precision, shared with reference selection) plus a 2x-downsampled FFT residual correlation with parabolic sub-pixel refinement (~0.05 px; scipy pocketfft at next-fast-length padding), then optional affine (rotation+scale) via star matching + RANSAC. Post-registration residuals are verified on a ~20% sample, escalating to all frames on failure. Handled by `run_registration_phase` in `registration.py`.
