...
2. **Phase 2 — Registration** — Coarse-to-fine pyramid seed (half-resolution, integer precision, shared with reference selection) plus a 2x-downsampled FFT residual correlation with parabolic sub-pixel refinement (~0.05 px; scipy pocketfft at next-fast-length padding), then optional affine (rotation+scale) via star matching + RANSAC. Post-registration residuals are verified on a ~20% sample, escalating to all frames on failure. Frames whose measured residual exceeds the threshold are dropped from the stack by default (`--no-reg-residual-reject` to keep them). The affine branch validates its own fit (shift < 10% of frame size, rotation < 5deg) before accepting it, falling back to translation-only registration otherwise -- a bad RANSAC star match can converge on a wildly wrong but internally-consistent transform, and that failure mode wasn't caught by any downstream check. Handled by `run_registration_phase` in `registration.py`.
