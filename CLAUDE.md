...
| [src/astrollm.py](src/astrollm.py) | astrollm integration (`--astrollm`): defect/quality/category scoring via a per-image subprocess call to a separately-trained model's `infer.py` (no network — local subprocess only). Scores every accepted light frame after Phase 1 (before `quality_gate`) and once more on the final stacked master. Advisory/logging only while the model is still finishing its first training run — results are stored in `FrameInfo.metrics['astrollm']` and logged (defective/stray-light flags, session-relative below-average `quality_score`, master category vs. the pipeline's own inferred target type) but never set `accepted` or feed `metrics['score']`, so nothing is auto-dropped |
